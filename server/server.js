...
function handle_upload(req, res) {
    ...
    busboy.on('file', function(fieldname, file, filename) {
        try {
            var ftmp = tmp.fileSync({ postfix: '.tmp', dir: req.app.locals.config.path.db, keep: true });
            tmpfname = ftmp.name;
...
function cf_do_invalidate(ident, mode, cfconfig) {
    var inv_url = mode + '://' + cfconfig.url + '/db/' + ident;
...
function create_app(config) {
  ...
  app.use('/db', express.static(config.path.db));
...
function init_defaults(config) {
  ...
  config.path.db = config.path.db ? config.path.db : "../db";
